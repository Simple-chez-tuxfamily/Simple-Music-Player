...
function afficher_message(message, temps, type) {
  if (!document.getElementById('notification') || message == null || temps == null || type == null) return false;
  if (type == 0) {
    document.getElementById('notification').style.borderTop = '4px solid #D63535';
    document.getElementById('notification').innerHTML = 'Erreur: ' + message;
  } else {
    document.getElementById('notification').style.borderTop = '4px solid #8BB548';
...
function masquer_message() {
  if (document.getElementById('notification')) $('#notification').fadeOut();
}